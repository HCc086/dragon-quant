...
class DataCache:
    ...
    def invalidate(self, key: str):
        ...
        if self._cache_dir:
            p = self._cache_dir / f"{self._safe_name(key)}.json"
            if p.exists():
                p.unlink()
    ...
    def _persist(self, key: str, data: Any):
        ...
        try:
            ...
            with open(self._cache_dir / f"{self._safe_name(key)}.json", "w") as f:
                json.dump({"data": safe, "ts": time.time()}, f, ensure_ascii=False)
        except Exception as e:
            print(f"  ⚠️缓存持久化失败 {key}: {e}", file=sys.stderr)
    ...
    @staticmethod
    def _safe_trade(key: str) -> str:
        return key.replace(":", "_")

    def _trade_date_path(self, key: str, trade_date: str, namespace: str = "") -> Optional[Path]:
        if not self._cache_dir:
            return None
        d = self._cache_dir / "provider" / trade_date
        suffix = f"__{namespace}" if namespace else ""
        return d / f"{self._safe_trade(key)}{suffix}.json"

    def set_for_trade_date(self, key: str, data: Any, trade_date: str, namespace: str = ""):
        with self._lock:
        ...
        try:
            ...
            print(f"  ⚠️交易日缓存写入失败 {key}: {e}", file=sys.stderr)

    def load_for_trade_date(self, key: str, trade_date: str, namespace: str = "") -> Optional[Any]:
        mem = self.get(key)
        if mem is not None:
            return mem
        p = self._trade_date_path(key, trade_date, namespace)
